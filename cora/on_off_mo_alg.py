...
class MultiValueMintermOnMo(MultiValueMintermOn):
    ...
    def __str__(self):
        return "{0}, {1},{2}".format(self.minterm, self.coverage, self.tag)
    ...
    def reduce_with_off_set(self, off_matrix):
        ...
        for ind, (elm, tag) in enumerate(zip(off_matrix[0], off_matrix[1])):
            ...
            if any(new_tag):
                ...
                res.append(MultiValueMintermOnMo(new_minterm, self.coverage, new_tag))
...
def on_off_grouping_mo(table, outputs):
    ...
    onset_index = data_output.apply(
        lambda row_series: any(x for x in row_series), axis=1
    )
    input_columns = [x for x in table.columns if x not in outputs]
    inputs = table[input_columns]
    onset_table = inputs[onset_index].apply(
        lambda row: tuple(int(x) if not np.isnan(x) else None for x in row), axis=1
    )
    tags_tuples = table[outputs].apply(tuple, axis=1)
    ind_on = [int(x) for x in table[onset_index].index]

    onset = [
        MultiValueMintermOnMo(row, {ind}, tags)
        for row, ind, tags in zip(onset_table, ind_on, tags_tuples[onset_index])
    ]

    offset_index = data_output.apply(
        lambda row_series: not all(x for x in row_series), axis=1
    )
...
def reduction_mo(onset, offset):
    ...
    for m in on_off_matrices:
        ...
        for current_tag in map(
            lambda x: number_to_tuple(x, tag_length), range(1, 2**tag_length)
        ):
            current_minterms = [
                mt
                for mt in m_reduced
