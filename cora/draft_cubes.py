...
def _get_levels(data, outputs, inputs):
    ...
    if len(inputs) == 1:
        ...
        dim = inputs.apply(
            lambda x: pd.unique(x).tolist(), axis=0, result_type="reduce"
        ).array
...
def _is_an_impicant(data, output, inputs, combination):
    ...
    return any(
        onset.apply(
            lambda row_series: (
                True
                if all(y == x for x, y in zip(row_series.values, combination) if y >= 0)
                else False
            ),
            axis=1,
        )
...
def _transform_to_raw_imp(impl, levels):
    s = {(ValuedVariable(i, v)) for i, v in enumerate(impl) if v != -1}
