...
class MultiValueMintermOn:
    ...
    def __str__(self):
        return "{0}, {1}".format(self.minterm, self.coverage)
    ...
    def __eq__(self, other):
        ...
        return self.minterm == other.minterm and self.coverage == other.coverage
    ...
    def _reduce(self, other):
        ...
        return MultiValueMintermOn(new_minterm, self.coverage.union(other.coverage))
        ...
        for ind, elm in enumerate(off_matrix.data):
            ...
            for i in range(0, n):
                ...
                if self.minterm[i] != elm[i]:
                    ...
                    new_minterm.append(int(-1))
            res.append(MultiValueMintermOn(new_minterm, self.coverage))
...
def _on_off_grouping(table, output, multi_output=False):
    ...
    onset = [MultiValueMintermOn(row, {ind}) for row, ind in zip(data_on, ind_on)]
...
def _reduction(onset, offset):
    ...
    for minterm in onset:
        on_off_matrixes.append(
            OnOffReductionMatrix(minterm._reduce_with_off_set(offset))
        )
