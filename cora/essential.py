...
def _reduce_group(group):
    return {
        reduce(_merge_elements, subgroup) for subgroup in group if len(subgroup) > 0
    }
...
def _included_in_offset(implicant, data, output):
    ...
    return any(
        offset.apply(
            lambda row_series: (
                True
                if all(y == x for x, y in zip(row_series.values, implicant) if y >= 0)
                else False
            ),
            axis=1,
        )
        ...
        cov_list.append(
            onset[
                onset.apply(
                    lambda row_series: (
                        True
                        if all(y == x for x, y in zip(row_series.values, imp) if y >= 0)
                        else False
                    ),
                    axis=1,
                )
...
def get_essential_implicants(data, output, levels):
    ...
    imps = [imp for imp in elimination if not _included_in_offset(imp, data, output)]
    return imps
