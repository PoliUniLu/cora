...
def _find_irredundant_sums(implicants_with_coverage, coverage, max_depth=None):
    ...
    results = []
    _find_irrendundant_sums_internal(
        implicants_with_coverage, [], set(), len(coverage), results, max_depth
    )
    return results


def _find_irrendundant_sums_internal(
    implicants_with_coverage,
    partial_solution,
    coverage,
    to_cover,
    all_solutions,
